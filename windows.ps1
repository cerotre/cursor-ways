...
Write-Host "$YELLOW   Follow us on WeChat [煎饼果子卷AI] $NC"
Write-Host "$YELLOW   Join our community for more Cursor tips and AI knowledge (script is free, follow us for more tips and experts)  $NC"
Write-Host "$YELLOW  [Important] This tool is free, if it helps you, please follow us on WeChat [煎饼果子卷AI]  $NC"
Write-Host "$BLUE================================$NC"
Write-Host ""
...
Write-Host "$YELLOW[Important]$NC Latest 0.49.x (supported)"
Write-Host ""
    ...
    try {
        ...
        if ($originalGuid) {
            ...
            if ($backupResult.ExitCode -eq 0) {
                Write-Host "$GREEN[Info]$NC Registry entry backed up to: $backupFile"
            } else {
        ...
        if (($backupFile -ne $null) -and (Test-Path $backupFile)) {
            ...
            if ($restoreResult.ExitCode -eq 0) {
                Write-Host "$GREEN[Restored]$NC Original registry value restored"
            } else {
                Write-Host "$RED[Error]$NC Restore failed, please manually import backup file: $backupFile"
            }
        } else {
            Write-Host "$YELLOW[Warning]$NC No backup file found or backup creation failed, cannot restore automatically"
        }
        return $false
    }
}
...
try {
    ...
    # Directly execute MachineGuid update, no longer asking
    Update-MachineGuid
    # Display results
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "$GREEN================================$NC"
    Write-Host "$YELLOW  Follow us on WeChat [煎饼果子卷AI] for more Cursor tips and AI knowledge (script is free, follow us for more tips and experts)  $NC"
    Write-Host "$GREEN================================$NC"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "$YELLOW[Question]$NC Do you want to disable Cursor auto-update?"
        ...
        try {
            ...
            try {
                ...
                if (-not $fileInfo.IsReadOnly) {
                    Write-Host "$RED[Error]$NC Verification failed: file permissions may not be set correctly"
                    Show-ManualGuide
                    return
                }
            }
...
if ($cursorVersion) {
    ...
    Write-Host "$YELLOW[Warning]$NC Unable to detect version, will continue..."
} 
